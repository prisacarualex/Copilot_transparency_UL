...
# ──────────────────────────────────────────────
# update_can_receive_key - manual override (shared human-AI control)
# ──────────────────────────────────────────────
class TestManualOverride:
    """Test allowmanualoverride enabling operator input while auto-solver runs."""

    def test_autosolver_disables_input_without_override(self, monkeypatch):
        """With the auto-solver on and no override, operator keys are disabled."""
        monkeypatch.setattr("plugins.abstractplugin.REPLAY_MODE", False)
        monkeypatch.setattr("plugins.sysmon.REPLAY_MODE", False)
        s = _make_sysmon()
        s.parameters["automaticsolver"] = True
        s.parameters["allowmanualoverride"] = False

        s.update_can_receive_key()

        assert s.can_receive_keys is False
        assert s.can_execute_keys is False

    def test_override_enables_input_despite_autosolver(self, monkeypatch):
        """With override on, operator may act on gauges even while auto-solver runs."""
        monkeypatch.setattr("plugins.abstractplugin.REPLAY_MODE", False)
        monkeypatch.setattr("plugins.sysmon.REPLAY_MODE", False)
        s = _make_sysmon()
        s.parameters["automaticsolver"] = True
        s.parameters["allowmanualoverride"] = True

        s.update_can_receive_key()

        assert s.can_receive_keys is True
        assert s.can_execute_keys is True

    def test_override_ignored_when_paused(self, monkeypatch):
        """A paused plugin never receives keys, even with override enabled."""
        monkeypatch.setattr("plugins.abstractplugin.REPLAY_MODE", False)
        monkeypatch.setattr("plugins.sysmon.REPLAY_MODE", False)
        s = _make_sysmon()
        s.parameters["automaticsolver"] = True
    ...
    def test_override_ignored_when_hidden(self, monkeypatch):
        """A hidden plugin never receives keys, even with override enabled."""
        monkeypatch.setattr("plugins.abstractplugin.REPLAY_MODE", False)
        monkeypatch.setattr("plugins.sysmon.REPLAY_MODE", False)
        s = _make_sysmon()
        s.parameters["automaticsolver"] = True
    ...
    def test_override_disabled_in_replay_mode(self, monkeypatch):
        """Replay mode suppresses live operator input regardless of override."""
        monkeypatch.setattr("plugins.abstractplugin.REPLAY_MODE", True)
        monkeypatch.setattr("plugins.sysmon.REPLAY_MODE", True)
        s = _make_sysmon()
        s.parameters["automaticsolver"] = True
        s.parameters["allowmanualoverride"] = True
