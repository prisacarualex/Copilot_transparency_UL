...
# ──────────────────────────────────────────────
# Keyboard control (_apply_keyboard_input)
# ──────────────────────────────────────────────
class TestKeyboardControl:
    def test_disabled_by_default_ignores_keys(self):
        """With keyboardcontrol off, WASD keys do not move the cursor."""
        t = _make_track()
        t.parameters["keyboardcontrol"] = False
        t._apply_keyboard_input("D", True)
        t._apply_keyboard_input("W", True)
        assert t.x_input == 0
        assert t.y_input == 0

    def test_wasd_press_sets_axis(self):
        """Each WASD key drives the expected axis when enabled."""
        t = _make_track()
        t.parameters["keyboardcontrol"] = True

        t._apply_keyboard_input("D", True)  # right
        assert t.x_input == 1.0
        t._apply_keyboard_input("A", True)  # left
        assert t.x_input == -1.0
        t._apply_keyboard_input("W", True)  # up (negative, per cursor convention)
        assert t.y_input == -1.0
        t._apply_keyboard_input("S", True)  # down
        assert t.y_input == 1.0

    def test_release_zeroes_axis(self):
        """Releasing a key returns that axis to neutral."""
        t = _make_track()
        t.parameters["keyboardcontrol"] = True
        t._apply_keyboard_input("D", True)
        assert t.x_input == 1.0
        t._apply_keyboard_input("D", False)
        assert t.x_input == 0.0

    def test_held_key_moves_cursor_without_joystick(self):
        """Holding a key should push the cursor away from centre over updates."""
        t = _make_track()
        t.parameters["keyboardcontrol"] = True
        t.parameters["joystickforce"] = 6
        t.reticle.is_cursor_in_target.return_value = True

        t._apply_keyboard_input("D", True)  # hold right
        positions = [next(t.cursor_path_gen) for _ in range(10)]
        # Cursor x should progress to the right (increasingly positive)
