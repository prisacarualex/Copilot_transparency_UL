...
class Track(AbstractPlugin):
    def __init__(
        ...
        self.validation_dict: dict[str, Callable[..., Any]] = {
            ...
            "keyboardcontrol": validation.is_boolean,
        }
        ...
        # Keys used to steer the cursor when keyboardcontrol is enabled
        # (WASD, so they do not clash with the Communications arrow keys).
        self.keys: set[str] = {"W", "A", "S", "D"}
    ...
    def do_on_key(self, key: str, state: str, emulate: bool = False) -> str | None:
        key = super().do_on_key(key, state, emulate)
        if key is None:
            return
        self._apply_keyboard_input(key, state == "press")
        return key

    def _apply_keyboard_input(self, key: str, pressed: bool) -> None:
        """Map WASD to joystick-like axis inputs (-1..1) when keyboardcontrol is on.

        A key hold pushes the cursor continuously (like holding a joystick axis);
        releasing the key zeroes that axis. Sign of the vertical axis matches the
        cursor convention (see compute_next_cursor_position: compy = -y_input).
        """
        if not self.parameters["keyboardcontrol"]:
            return
        if key == "W":
            self.y_input = -1.0 if pressed else 0.0
        elif key == "S":
            self.y_input = 1.0 if pressed else 0.0
        elif key == "A":
            self.x_input = -1.0 if pressed else 0.0
        elif key == "D":
            self.x_input = 1.0 if pressed else 0.0
