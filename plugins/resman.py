        ...
        if self.wait_before_leak > 0:
            ...
            if self.parameters["automaticsolver"] is True:
                for pump_n, this_pump in {p: v for p, v in pumps.items() if v["state"] != "failure"}.items():
                    ...
                    prev_state: str = this_pump["state"]
                    ...
                    # Explain any state change to the copilot panel
                    if this_pump["state"] != prev_state:
                        new_state: str = this_pump["state"]
                        if new_state == "on":
                            reason = (
                                f"Tank {this_pump['_totank'].upper()} level "
                                f"({int(to_tank['level'])} units) is low — filling from "
                                f"Tank {this_pump['_fromtank'].upper()}"
                            )
                        else:
                            reason = (
                                f"Tank {this_pump['_totank'].upper()} level "
                                f"({int(to_tank['level'])} units) is sufficient — halting "
                                f"Pump {pump_n}"
                            )
                        self._explain_pump_change(pump_n, new_state, reason)
